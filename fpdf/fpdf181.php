...
function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	$this->StdPageSizes = array(
		...
		'cfs'=>array(396.85,615.11), 
		'cfm'=>array(615.11,793.70), 
		'cfb'=>array(793.70,992.13)  
		);
...
function SetDash($black=null, $white=null)
 ...
 function Add_Page($orientation='P', $size='A4', $max_lenght=300)
{	
	...
	switch ($orientation){
		case 'P' :
			switch ($size){
				case 'A4' :
					$nWidth = Max(210,$max_lenght);
					$nHeight = 297 / 210 * $nWidth  ;
					break ;
				case 'A3' :
					$nWidth  = Max(420,$max_lenght);
					$nHeight = 420 /297 * $nWidth  ; 
					break ;
				case 'A5' :
					$nWidth  = Max(210,$max_lenght);
					$nHeight = 210 / (297 /2) * $nWidth  ; 
					break ;
				case 'CFS' :
					$nWidth  = Max(280,$max_lenght);
					$nHeight = 140 / 280 * $nWidth  ; 
					break ;
				case 'CFM' :
					$nWidth  = Max(280,$max_lenght);
					$nHeight = 215 / 280 * $nWidth  ; 
					break ;
			}
			break ;
		case 'L' :
			switch ($size){
				case 'A4' :
					$nWidth = Max(297,$max_lenght);
					$nHeight=  210 / 297    * $nWidth  ;
					break ;
				case 'A3' :
					$nWidth  = Max(420,$max_lenght);
					$nHeight = 297/ 420 * $nWidth  ; 
					break ;
				case 'A5' :
					$nWidth = Max(210,$max_lenght);
					$nHeight = (297/2) / 210 * $nWidth  ;
					break ;
				case 'CFS' :
					$nHeight = Max(140,$max_lenght); ;
					$nWidth =  140/215 * $nHeight;
					break ;
				case 'CFM' :
					$nHeight = Max(280,$max_lenght); ;
					$nWidth =  140/215 *  $nHeight;
					break ;
			}
			break ;	
	}
	$this->AddPage($orientation,array($nWidth,$nHeight));
	
}

public function abox($kotak){ 
	/*
	$kotak = array(10,30,15,array(50,20,30,15,15,25,15,10,5,25,40));
	$kotak = array(x, y, z , lebar kolom)
	*/
		$lebar = $kotak[3] ;
		$x1 = $kotak[0] ;
		$x2 = $x1 ;
...
public function acell(     $x1, $y1, $w , $h, $text, $border = 0 , $ln = 0 , $align = 'L', $afill = false  ){ $this->setXY($x1, $y1) ; if ($this->GetStringWidth($text)+1 < $w){ $this->Cell($w, $h, $text,$border ,$ln, $align ,$afill); } else { if ($border == 1){ $this->Rect($x1, $y1,$w , $h ); } $nH = ($h*0.52) ; $this->MultiCell($w, $nH, $text ,0, $align); } }	
public function aMultiCell($x1, $y1, $w , $h, $text, $border = 0 , $ln = 6 , $align = 'L', $afill = false  ){ 
	$this->setXY($x1, $y1) ; 
	if ($border == 1){ 
		$this->Rect($x1, $y1,$w , $h ); 
	} 
	$this->MultiCell($w, $ln, $text ,0, $align); 

}	
public function aline($x1, $y1, $nP) { $this->Line($x1, $y1 , $x1 + $nP ,$y1) ; }	
public function vline($x1, $y1, $nP) { $this->Line($x1, $y1 , $x1  ,$y1+ $nP) ; }
...
function PutLink($URL, $txt)
{
 ...
 function WriteHtmlCell($xrm , $xlm, $html){  
	$this->SetX(0);
    $rm = $this->rMargin;
    $lm = $this->lMargin;
    $this->SetRightMargin($xrm);
    $this->SetLeftMargin($xlm);
    $this->WriteHtml($html);
