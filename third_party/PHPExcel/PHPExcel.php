...
class PHPExcel
{
    ...
    */
	...
	public function initial($ctitle , $user='user', $cSubject='',$cDescription='',$cKeywords=''){
		$this->getProperties()->setCreator( $user)
	...
	public function sheet_name($nsheet = 0, $cname){
		
		$this->getActiveSheet($nsheet)->setTitle($cname ); 
		$this->getActiveSheet($nsheet)->setShowGridlines(False); 
	}
	
	public function set_width($kolom,$lebar){
		$this->getActiveSheet()->getColumnDimension($this->RowToCol($kolom))->setWidth($lebar);  
	}
	
	function Outputxlsx($file_name){
	...
	function Output($file_name,$ext='xls'){ 
	  
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="'.$file_name.'.'.$ext.'"');  
		header('Cache-Control: max-age=0');
		//ob_end_clean();
		$write = PHPExcel_IOFactory::createWriter($this,($ext=='xlsx' ? 'Excel2007' :'Excel5' ) );
		$write->save('php://output');
		 
		exit ;
	}
	
	public function marge_cell($nsheet = 0,$cCell, $cCenter ='Y'){
		$this->setActiveSheetIndex($nsheet)->mergeCells($cCell);
		switch ($cCenter) {
			case 'Y' :
			case 'A' :
				$this->getActiveSheet()->getStyle($cCell)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->getActiveSheet()->getStyle($cCell)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
				break ;
			case 'V' :
				$this->getActiveSheet()->getStyle($cCell)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
				break ;
	...
	public function marge($nsheet = 0,$kolom1, $baris1, $kolom2, $baris2, $cCenter ='Y'){
		$cCell = $this->RowToCol($kolom1).$baris1.':'.$this->RowToCol($kolom2).$baris2 ;
		$this->setActiveSheetIndex($nsheet)->mergeCells($cCell);
		switch ($cCenter) {
			case 'Y' :
			case 'A' :
	...
	public function cell($nsheet = 0, $kolom,$baris,$kata,$lbold = false, $nfont = 10, $lborder = 0){
		$this->setActiveSheetIndex($nsheet)->setCellValue($this->RowToCol($kolom).$baris, "$kata");
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setBold($lbold );  
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setSize($nfont ); 
		if ($lborder == 1){
			$BStyle = array(
				  'borders' => array(
					'allborders' => array(
			...
			$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->applyFromArray($BStyle);
		}
	}
	 
	public function text($nsheet = 0, $kolom,$baris,$kata,$lbold = false, $nfont = 10, $lborder = 0){
		$this->setActiveSheetIndex($nsheet)->setCellValue($this->RowToCol($kolom).$baris, "$kata");
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setBold($lbold );  
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setSize($nfont ); 
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getNumberFormat()->setFormatCode('@');
		if ($lborder == 1){
			$BStyle = array(
				  'borders' => array(
					'allborders' => array(
	...
	public function number($nsheet = 0, $kolom,$baris,$kata,$lbold = false, $nfont = 10, $lborder = 0){
		$this->setActiveSheetIndex($nsheet)->setCellValue($this->RowToCol($kolom).$baris, "$kata");
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setBold($lbold );  
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setSize($nfont ); 
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getNumberFormat()->setFormatCode('#,##0.00');
		if ($lborder == 1){
			$BStyle = array(
				  'borders' => array(
					'allborders' => array(
	...
	public function number0($nsheet = 0, $kolom,$baris,$kata,$lbold = false, $nfont = 10, $lborder = 0){
		$this->setActiveSheetIndex($nsheet)->setCellValue($this->RowToCol($kolom).$baris, "$kata");
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setBold($lbold );  
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getFont()->setSize($nfont ); 
		$this->getActiveSheet()->getStyle($this->RowToCol($kolom).$baris)->getNumberFormat()->setFormatCode('#,##0');
		if ($lborder == 1){
			$BStyle = array(
				  'borders' => array(
					'allborders' => array(
	...
	public function RowToCol($angka){ 
		if (($angka > 26 ) and ($angka <= 52 )) {
			$angka1 = $angka - 26 ;
			$kolom = chr(64+1 ) .chr(64 + $angka1 ) ;
		} else if (($angka > 52) and ($angka <= 78 )){
			$angka1 = $angka - 52 ;
			$kolom = chr(64 + 2 ) .chr(64 + $angka1 ) ;
		} else if (($angka > 78) and ($angka <= 104 )){
			$angka1 = $angka - 78 ;
			$kolom = chr(64 + 3 ) .chr(64 + $angka1 ) ;
		} else {
			$kolom = chr(64 + $angka);
		}
		return $kolom  ;
